...
public class TerraformBuildWrapper extends BuildWrapper {
	...
	public TerraformBuildWrapper(String workspace, String variables, String terraformInstallation, boolean doGetUpdate,
			boolean doNotApply, boolean doDestroy, Configuration config) {
	...
	public TerraformInstallation getInstallation() {
		for (TerraformInstallation installation : ((DescriptorImpl) getDescriptor()).getInstallations()) {
			if (terraformInstallation != null && installation.getName().equals(terraformInstallation)) {
				return installation;
			}
		}
		return null;
	...
	public String getExecutable(EnvVars env, BuildListener listener, Launcher launcher)
			throws IOException, InterruptedException {
		String executablePath = null;
		try {
			TerraformInstallation terraform = getInstallation().forNode(Computer.currentComputer().getNode(), listener)
					.forEnvironment(env);
	...
	public void executeInit(AbstractBuild build, final Launcher launcher, final BuildListener listener)
			throws Exception {
	...
	public void executeApply(AbstractBuild build, final Launcher launcher, final BuildListener listener)
		...
		if (!isNullOrEmpty(getVariables())) {
			variablesFile = workingDirectory.createTextTempFile("variables", ".tfvars",
					evalEnvVars(getVariables(), env));
	...
	public Environment setUp(AbstractBuild build, final Launcher launcher, final BuildListener listener)
		...
		try {
			...
			/*
			 * if (! doNotApply) { executeApply(build, launcher, listener); }
			 */
			...
			public boolean tearDown(AbstractBuild build, BuildListener listener)
				...
				if (doDestroy()) {
					...
					try {
						...
						int result = launcher.launch().pwd(workspacePath.getRemote()).cmds(args).stdout(listener)
								.join();
